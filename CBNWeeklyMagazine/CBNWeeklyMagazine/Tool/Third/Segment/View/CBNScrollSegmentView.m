...
#import "CBNSegmentLabel.h"
@interface CBNScrollSegmentView ()
{
...
// 所有标题的设置
@property (strong, nonatomic) CBNSegmentStyle *segmentStyle;
// 所有的标题
...
// 响应标题点击
@property (copy, nonatomic) SegmentModelClickedBlock titleBtnOnClick;
...
@implementation CBNScrollSegmentView
- (instancetype)initWithFrame:(CGRect )frame segmentStyle:(CBNSegmentStyle *)segmentStyle titles:(NSArray *)titles titleDidClick:(SegmentModelClickedBlock)segmentItemClick
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.titles = titles;
        self.titleBtnOnClick = segmentItemClick;
        _currentIndex = 0;
        _oldIndex = 0;
        _currentWidth = frame.size.width;
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(segmentChanged:) name:@"segmentChanged" object:@"segmentChanged"];
...
- (void)dealloc
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"segmentChanged" object:@"segmentChanged"];
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    CBNSegmentLabel *currentLabel = (CBNSegmentLabel *)tapGes.view;
...
- (void)setupTitles {
    ...
    NSInteger index = 0;
    for (CBNSegmentModel *segmentItem in self.titles) {
        NSLog(@"%@",segmentItem.title);
        CBNSegmentLabel *label = [[CBNSegmentLabel alloc] initWithFrame:CGRectZero];
        label.tag = index;
...
//        label.backgroundColor = [UIColor orangeColor];
        ...
        CGRect bounds = [segmentItem.title boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: label.font} context:nil];
...
- (void)setupUI {
    ...
    if (self.segmentStyle.isScrollTitle) { // 设置滚动区域
        CBNSegmentLabel *lastLabel = (CBNSegmentLabel *)self.titleLabels.lastObject;
...
- (void)setUpLabelsPosition {
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        NSInteger index = 0;
        for (CBNSegmentLabel *label in self.titleLabels) {
        ...
        for (CBNSegmentLabel *label in self.titleLabels) {
            ...
            if (index != 0) {
                CBNSegmentLabel *lastLabel = (CBNSegmentLabel *)self.titleLabels[index - 1];
    ...
    CBNSegmentLabel *firstLabel = (CBNSegmentLabel *)self.titleLabels[0];
...
//        }
    ...
    CBNSegmentLabel *oldLabel = (CBNSegmentLabel *)self.titleLabels[_oldIndex];
    CBNSegmentLabel *currentLabel = (CBNSegmentLabel *)self.titleLabels[_currentIndex];
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    CBNSegmentLabel *oldLabel = (CBNSegmentLabel *)self.titleLabels[oldIndex];
    CBNSegmentLabel *currentLabel = (CBNSegmentLabel *)self.titleLabels[currentIndex];
