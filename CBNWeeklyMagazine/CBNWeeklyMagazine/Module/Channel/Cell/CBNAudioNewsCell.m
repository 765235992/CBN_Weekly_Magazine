...
@implementation CBNAudioNewsCell
...
- (CBNImageView *)audioIconImageView
{
    if (!_audioIconImageView) {
        ...
        self.audioIconImageView = [[CBNImageView alloc] initWithFrame:CGRectMake(10, 8, 26*screen_Width/320, 29*screen_Width/320)];
...
- (CBNImageAndTextLabel *)audioTimeLabel
{
    if (!_audioTimeLabel) {
        ...
        self.audioTimeLabel = [[CBNImageAndTextLabel alloc] initWithFrame:CGRectMake(0, 0, 0, 0) image:[RGBColor(102, 198, 118, 1.0) colorImage]];
...
{
    NSLog(@"%@",newsItemModel.cover_img_big);
    ...
    _audioTimeLabel.text = [NSString stringWithFormat:@"%@",newsItemModel.Time];
